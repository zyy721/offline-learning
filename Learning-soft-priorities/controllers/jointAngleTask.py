...
class jointAngleTask:
    ...
    def __init__(self, skel, setPoint, taskWeight=None, selectionVector=None, Kd=None, Kp=None, bodyNodeIndex=None):
        ...
        if setPoint is None:
            ...
            self.desiredJointAngle = setPoint
        ...
        if selectionVector is None:
            ...
            self.selectionMatrix = np.identity(setPoint.shape[0])
            for i in range(setPoint.shape[1]):
    ...
    def cal_QP_Matricies(self):
        ...
        newJacobian = np.identity(self.robot.ndofs)
        ...
        newJacobian_dot = np.zeros((self.robot.ndofs,self.robot.ndofs))
        ...
        self.error = self.selectionMatrix.dot(q - self.desiredJointAngle)
